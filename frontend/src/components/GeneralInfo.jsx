function GeneralInfo({ data, setData, setStep, setShowForm, onSaveChanges, isUpdating, enhancingId, onEnhance }) {
  ...
  return (
      <div className="border md:border-r-20 rounded-br-4xl rounded-bl-4xl p-10 mt-0 w-full max-w-3xl border-yellow-600 ">
        ...
        <input type="number" placeholder="Phone" value={data.phone} onChange={(e) => setData({ ...data, phone: e.target.value })} className="block w-full border p-3 mb-10 rounded italic active:border-yellow-600 focus:border-yellow-600 text-black dark:bg-black dark:text-stone-50" required />
        <input type="text" placeholder="Github Username (optional)" value={data.github} onChange={(e) => setData({ ...data, github: e.target.value })} className="block w-full border p-3 mb-10 rounded italic active:border-yellow-600 focus:border-yellow-600 text-black dark:bg-black dark:text-stone-50" />
        <input type="url" placeholder="LinkedIn Profile URL (optional)" value={data.linkedin} onChange={(e) => setData({ ...data, linkedin: e.target.value })} className="block w-full border p-3 mb-10 rounded italic active:border-yellow-600 focus:border-yellow-600 text-black dark:bg-black dark:text-stone-50" />
        
        <div className="mb-10">
          <textarea placeholder="About Yourself (optional)" value={data.about} onChange={(e) => setData({ ...data, about: e.target.value })} className="block w-full border p-3 rounded italic active:border-yellow-600 focus:border-yellow-600 text-black dark:bg-black dark:text-stone-50 min-h-[120px]" />
